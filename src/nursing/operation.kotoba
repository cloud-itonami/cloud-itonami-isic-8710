(ns nursing.operation
  "OperationActor -- one nursing operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (NursingOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Nursing Care Governor (:govern) and the rollout
  ...
  One graph run = one nursing operation (intake -> advise -> govern ->
  ...
  decision to a human operator (licensed nursing staff). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/administer-medication`/`:actuation/finalize-
  incident-response` ALWAYS reach this node when the governor is clean
  -- see `nursing.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [nursing.nursingadvisor :as nursingadvisor]
            [nursing.governor :as governor]
...
(defn build
  ...
  `nursing.store/Store`).
  opts:
    :advisor      -- a `nursing.nursingadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (nursingadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NursingOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (nursingadvisor/-advise advisor store request)]
            {:proposal p :audit [(nursingadvisor/trace request p)]})))

      ;; Nursing Care Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
